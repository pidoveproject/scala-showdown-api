...
import io.github.pidoveproject.showdown.protocol.server.TournamentMessage
import io.github.pidoveproject.showdown.protocol.server.tournament.*
import io.github.pidoveproject.showdown.testing.protocol.assertDecodeString
...
object TournamentSuite extends TestSuite:
  val tests = Tests:
    ...
    test("create") - assertDecodeString(
      decoder,
      "|tournament|create|gen9monotype|Single Elimination|128",
    ...
    test("update"):
      ...
      test("users") - assertDecodeString(
        decoder,
        """|tournament|update|{"bracketData":{"type":"Tree","rootNode":null,"users":["KeshBa45","Leeko","Olekpop1","we up"]}}""",
        TournamentMessage.Update(TournamentUpdate(
      ...
      test("settings") - assertDecodeString(
        decoder,
        """|tournament|update|{"format":"gen9monotype","generator":"Single Elimination","playerCap":128,"isStarted":false,"isJoined":false}""".stripMargin,
        TournamentMessage.Update(TournamentUpdate(
      ...
      test("ingame") - assertDecodeString(
        decoder,
        """|tournament|update|{"bracketData":{"type":"tree","rootNode":{"children":[{"children":[{"team":"Karan Goyal#2432"},{"team":"Ray22-1"}],"state":"available"},{"children":[{"team":"Zarel"},{"team":"Il_totore"}],"state":"inprogress"}],"state":"unavailable"}}}""",
        TournamentMessage.Update(TournamentUpdate(
          bracketData = Some(BracketData(
            bracketType = BracketType.Tree,
    ...
    test("replace") - assertDecodeString(
      decoder,
      "|tournament|replace|UnderSkyle|Iltotore",
      TournamentMessage.Replace(
        Username("UnderSkyle"),
        Username("Iltotore")
      )
    )
    ...
    test("battleStart") - assertDecodeString(
      decoder,
      "|tournament|battlestart|UnderSkyle|Iltotore|battle-gen7randombattle-1919195988",
      TournamentMessage.BattleStart(
        Username("UnderSkyle"),
        Username("Iltotore"),
        RoomId("battle-gen7randombattle-1919195988")
      )
    )
    test("battleEnd") - assertDecodeString(
      decoder,
      "|tournament|battleend|P4bl00|angeleri|loss|0,1|success|battle-gen9doublesou-1919196310",
      TournamentMessage.BattleEnd(
        Username("P4bl00"),
        Username("angeleri"),
        BattleResult.Loss(),
    ...
    test("end") - assertDecodeString(
      decoder,
      """|tournament|end|{"results":[["trichotomy"]],"format":"gen9monotype","generator":"Single Elimination","bracketData":{"type":"tree","rootNode":{"children":[{"children":[{"team":"Karan Goyal#2432"},{"team":"Ray22-1"}],"state":"available"},{"children":[{"team":"Zarel"},{"team":"Il_totore"}],"state":"unavailable"}],"state":"unavailable"}}}""",
      TournamentMessage.End(TournamentEnd(
    ...
    test("autoDq"):
      ...
      test("target") - assertDecodeString(
        decoder,
        "|tournament|autodq|target|1000",
        TournamentMessage.AutoDq(TournamentAutoDq.Target(Timestamp(1000)))
      )
